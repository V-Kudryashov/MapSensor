...
namespace VK.MapSensor
{
    ...
    public class TerrainMap : MonoBehaviour
    {
        ...
        public void Init(bool hardInit = false)
        {
            if (map != null && !hardInit)
                return;
        ...
        public void fillFrame(Transform cam, float[,,] frame)
        {
            fillFrameCPU(cam, frame);
        }
        private void fillFrameCPU(Transform cam, float[,,] frame)
        {
            float posXtoMap = (MapResolution - 1) / size.x;
            float posZtoMap = (MapResolution - 1) / size.z;

            Vector3 pos = terrain.transform.InverseTransformPoint(cam.position);
            float cX = pos.x * posXtoMap;
            float cY = pos.z * posZtoMap;

            Vector3 forward = terrain.transform.InverseTransformVector(cam.forward);
            forward.y = 0;
